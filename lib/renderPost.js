...
<body>
  ...
  <script>
    ...
    // Image grid: multiple images in same paragraph → flex row
    const postBody = document.querySelector(".post-body");
    postBody.querySelectorAll("p").forEach(function(p) {
      const nodes = Array.from(p.childNodes).filter(function(n) { return !(n.nodeType === Node.TEXT_NODE && !n.textContent.trim()); });
      if (nodes.length > 1 && nodes.every(function(n) { return n.nodeName === "IMG"; })) p.classList.add("img-row");
    });
    var isSingleImageP = function(p) {
      const nodes = Array.from(p.childNodes).filter(function(n) { return !(n.nodeType === Node.TEXT_NODE && !n.textContent.trim()); });
      return nodes.length === 1 && nodes[0].nodeName === "IMG";
    };
    var allP = Array.from(postBody.querySelectorAll("p"));
    var gi = 0;
    while (gi < allP.length) {
      if (isSingleImageP(allP[gi])) {
        var gj = gi + 1;
        while (gj < allP.length && isSingleImageP(allP[gj]) && allP[gj].previousElementSibling === allP[gj - 1]) gj++;
        if (gj > gi + 1) {
          var row = document.createElement("div");
          row.className = "img-row";
          allP[gi].parentNode.insertBefore(row, allP[gi]);
          for (var gk = gi; gk < gj; gk++) {
            row.appendChild(allP[gk].firstElementChild);
            allP[gk].remove();
          }
        }
        gi = gj;
